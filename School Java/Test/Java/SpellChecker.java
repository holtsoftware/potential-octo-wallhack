...
public class SpellChecker 
{
    ...
    public boolean check_in_dict(String dict_file,String the_word)
    {
        ...
        catch(FileNotFoundException exception)
        {
            ...
            System.out.println("File: " + dict_file);
        }
    ...
    public boolean check_word(String the_word)
    {
        ...
        if(!((which_dict >= 'A' && which_dict <= 'Z') || (which_dict >= 'a' && which_dict <= 'z')))
            return true;
        pwords.clear();
        String dict_file=new String(path + '\\' + which_dict + '.' + file_ext);
        boolean found = check_in_dict(dict_file,the_word);
        dict_file = path + "\\custom." + file_ext;
        if(!found)
            found = check_in_dict(dict_file,the_word);
        return found;
    }
    ...
    {
        final String CUST_DICT = new String(path + "\\custom." + file_ext);
        Vector c_dict=new Vector();
        String line;
        
        try
        {
            FileReader fr = new FileReader(CUST_DICT);
            BufferedReader inFile = new BufferedReader (fr);
        ...
        c_dict.addElement(word);
        try
        {
            FileWriter fw = new FileWriter(CUST_DICT);
            BufferedWriter bw = new BufferedWriter (fw);
            PrintWriter outFile = new PrintWriter (bw);
        
            int size = c_dict.size();
            for(int i=0;i<size;i++)
            {
                outFile.println(((String) c_dict.elementAt(i)));
            }
            outFile.close();
        }
        catch(FileNotFoundException exception)
