...
IMPLEMENT_DYNAMIC(CElevDlg, CDialog);
...
void CElevDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CElevDlg::OnQueryDragIcon()
{
...
void CElevDlg::OnCancel() 
{
	if (CanExit())
		CDialog::OnCancel();
}

BOOL CElevDlg::CanExit()
{
...
void CElevDlg::OnButton1() 
{
	...
	d.x=pas.x-to.x;
	d.y=pas.y-to.y;
	while(to.x<=pas.x&&to.y<pas.y)
	{
		dc.SetPixel(to,RGB(255,89,100));
		to.x-=d.x;
		to.y-=d.y;
	}
}
